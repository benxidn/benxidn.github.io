/* ### MAIN LINK ### */
/*DV SC 32bit*/
function MainDS() { window.open("https://ponselharian.com/OEha","_self"); }
/*Sc DV 1X 32bit*/
function MainL1() { window.open("https://cararegistrasi.com/cuRGoDTN","_self"); }
/*Sc DV 2X 32bit*/
function MainL2() { window.open("https://cararegistrasi.com/lWkmDbjpnU","_self"); }
/*Sc DV 3X 32bit*/
function MainL3() { window.open("https://cararegistrasi.com/DGUC4xey7H","_self"); }
/*Sc DV 4X 32bit*/
function MainL4() { window.open("https://cararegistrasi.com/pckDLIMe","_self"); }
/*Sc DV 5X 32bit*/
function MainL5() { window.open("https://cararegistrasi.com/JvWnsAly","_self"); }
/*Sc DV 6X 32bit*/
function MainL6() { window.open("https://cararegistrasi.com/SYfqTJ","_self"); }
/*Sc DV 7X 32bit*/
function MainL7() { window.open("https://cararegistrasi.com/A3Vuw","_self"); }
/* ########## */
/*DV SC 64bit*/
function MainDS64() { window.open("https://ponselharian.com/kYVLhlDsuXo","_self"); }
/*Sc DV 1X 64bit*/
function MainL1X64() { window.open("https://cararegistrasi.com/Ab5Q","_self"); }
/*Sc DV 2X 64bit*/
function MainL2X64() { window.open("https://cararegistrasi.com/RrkadaMqC40","_self"); }
/*Sc DV 3X 64bit*/
function MainL3X64() { window.open("https://cararegistrasi.com/P3Td3zDHwUTq","_self"); }
/*Sc DV 4X 64bit*/
function MainL4X64() { window.open("https://cararegistrasi.com/hxcM0LY","_self"); }
/*Sc DV 5X 64bit*/
function MainL5X64() { window.open("https://cararegistrasi.com/mdIUPstBRgh","_self"); }
/*Sc DV 6X 64bit*/
function MainL6X64() { window.open("https://cararegistrasi.com/Z0e8Xv","_self"); }
/*Sc DV 7X 64bit*/
function MainL7X64() { window.open("https://cararegistrasi.com/kCuT9","_self"); }

/*DV APK 32bit*/
function MainDAS() { window.open("https://ponselharian.com/JHtO1yt4","_self"); }
/* ########## */
/*DV APK 64bit*/
function MainDA() { window.open("https://ponselharian.com/d4BUfY","_self"); }

/*DV APK IG*/
function MainDI() { window.open("https://ponselharian.com/lIw3","_self"); }

/*LBX MU*/
function MainLN() { window.open("https://ponselharian.com/tpTf","_self"); }
/*Sc LMU 2X*/
function MainSFr2() { window.open("https://cararegistrasi.com/nOtZxA3","_self"); }
/*Sc LMU 3X*/
function MainSFr3() { window.open("https://cararegistrasi.com/ZMUp0KW9ii","_self"); }
/*Sc LMU 4X*/
function MainSFr4() { window.open("https://cararegistrasi.com/u07ao","_self"); }
/*Sc LMU 5X*/
function MainSFr5() { window.open("https://cararegistrasi.com/uNG6iWa","_self"); }
/*Sc LMU 6X*/
function MainSFr6() { window.open("https://cararegistrasi.com/nmuivok","_self"); }
/*Sc LMU 7X*/
function MainSFr7() { window.open("https://cararegistrasi.com/Dyfcp92","_self"); }
/*Sc LMU BU*/
function MainSFrBu() { window.open("https://cararegistrasi.com/xm3NbrDFe0t","_self"); }

/*LBX AU*/
function MainLAu() { window.open("https://ponselharian.com/2XVncf8","_self"); }
/*Sc LAU 1X*/
function MainS1() { window.open("https://semawur.com/xFF3KPNz","_self"); }
/*Sc LAU 2X*/
function MainS2() { window.open("https://cararegistrasi.com/JQod","_self"); }
/*Sc LAU 3X*/
function MainS3() { window.open("https://cararegistrasi.com/G67lp","_self"); }
/*Sc LAU 4X*/
function MainS4() { window.open("https://cararegistrasi.com/8niZ1MbPC4","_self"); }
/*Sc LAU 5X*/
function MainS5() { window.open("https://cararegistrasi.com/YwTYqa","_self"); }
/*Sc LAU BU*/
function MainBU() { window.open("https://cararegistrasi.com/9UPml3k","_self"); }

/*MH AA SKIN (Lite)*/
function MainMAS() { window.open("https://ponselharian.com/VqhT4fvE","_self"); }
/* ########## */
/*MH AA (Apk)*/
function MainMA() { window.open("https://ponselharian.com/sMgAQ0zIKt","_self"); }

/*MH KN*/
function MainMK() { window.open("https://ponselharian.com/sbKZPhHM","_self"); }

/*MH FC*/
function MainMF() { window.open("https://ponselharian.com/lRVtT52","_self"); }

/*MH TD*/
function MainMT() { window.open("https://ponselharian.com/wdPWThb0","_self"); }

/*UAS APK*/
function MainIS() { window.open("https://ponselharian.com/A7f7Vp40Hv","_self"); }

/*RB APK*/
function MainRA() { window.open("https://ponselharian.com/YkNZuD","_self"); }

/*RB SC*/
function MainRS() { window.open("https://ponselharian.com/2cEix3","_self"); }

/*DU SC*/
function MainDuS() { window.open("https://ponselharian.com/8v19aOttqP","_self"); }

/*HOOK SC*/
function MainHS() { window.open("https://ponselharian.com/QuYMQp","_self"); }

/*MORE LINK*/
function ZArchiver() { window.open("https://ponselharian.com/xRFvrfw8CE"); }
function MTManager() { window.open("https://ponselharian.com/toNp7"); }
function Brave() { window.open("https://ponselharian.com/nnhAB"); }

/* ########## */

/* ### MORE LINK ### */
function MoreKFc() {
  window.open("https://ponselharian.com/0aOycce"); }
function MoreKLb() {
  window.open("https://ponselharian.com/Muk1vnBum"); }

/* ### MAPHACK AA ### */
function MhAaSkin() {
  window.open("https://ponselharian.com/lRvY5sN9wg"); }
function SubMAS() {
  window.open("https://cararegistrasi.com/6DI142MLg","_self"); }

function MhAa() {
  window.open("https://ponselharian.com/TKti"); }
function SubMA() {
  window.open("https://cararegistrasi.com/UYfeD77zWDvu","_self"); }

/* ### MAPHACK KN ### */
function MhKn() {
  window.open("https://ponselharian.com/xV8a6G8LX2"); }
function SubMK() {
  window.open("https://cararegistrasi.com/EvOfOy3Lcn0","_self"); }

/* ### MAPHACK FC ### */
function MhFc() {
  window.open("https://ponselharian.com/M2DNtoZJ"); }
function SubMF() {
  window.open("https://cararegistrasi.com/AvIwkCNss","_self"); }

function MoreKFc() {
  window.open("https://ponselharian.com/0aOycce","_self"); }

/* ### MAPHACK TD ### */
function MhTd() {
  window.open("https://ponselharian.com/Hyc7"); }
function SubMT() {
  window.open("https://cararegistrasi.com/EOrJuBMyoyuf","_self"); }

/* ### DV APK ### */
function DvApkSkin() {
  window.open("https://ponselharian.com/4d21"); }
function SubDAS() {
  window.open("https://cararegistrasi.com/wjPyDa","_self"); }

function DvApk() {
  window.open("https://ponselharian.com/xzrE0QGi"); }
function SubDA() {
  window.open("https://cararegistrasi.com/DsAELZ4O","_self"); }

/* ### DV APK IG ### */
function DvIg() {
  window.open("https://ponselharian.com/nE8NkZXZ"); }
function SubDI() {
  window.open("https://semawur.com/roGgS","_self"); }

function MoreKIg() {
  window.open("https://ponselharian.com/s4rKVVAp"); }

/* ### DV SC ### */
function DvSc() {
  window.open("https://ponselharian.com/nHNKtR5kG"); }
function SubDS() {
  window.open("https://cararegistrasi.com/z5likCtCvF","_self"); }

function Lib1() {
  window.open("https://ponselharian.com/0hkk66vFZ"); }

function Lib2() {
  window.open("https://ponselharian.com/VRxWQUj"); }

function Lib3() {
  window.open("https://ponselharian.com/25UYvOeemAs"); }

function Lib4() {
  window.open("https://ponselharian.com/sYhpgICs"); }

function Lib5() {
  window.open("https://ponselharian.com/ubCpBsCZT"); }

function Lib6() {
  window.open("https://ponselharian.com/QaH6"); }

function Lib7() {
  window.open("https://ponselharian.com/nxJJUgM"); }

function DvSc64() { /*64 BIT*/
  window.open("https://ponselharian.com/NZQfy2Kmk"); }
function SubDS64() {
  window.open("https://cararegistrasi.com/DSE6Xz6tDVS","_self"); }

function Lib1X64() {
  window.open("https://ponselharian.com/8UQOExKW4Fz"); }

function Lib2X64() {
  window.open("https://ponselharian.com/8gbF2CarZa"); }

function Lib3X64() {
  window.open("https://ponselharian.com/8IaLyiNui3"); }

function Lib4X64() {
  window.open("https://ponselharian.com/slm3P"); }

function Lib5X64() {
  window.open("https://ponselharian.com/aRwe4"); }

function Lib6X64() {
  window.open("https://ponselharian.com/Mfmnb"); }

function Lib7X64() {
  window.open("https://ponselharian.com/js61B"); }

/* ### LULUBOX AU ### */
function LbAu() {
  window.open("https://ponselharian.com/0VJ"); }
function SubLAu() {
  window.open("https://cararegistrasi.com/0H7cBZfC0","_self"); }

function MoreKLb() {
  window.open("https://ponselharian.com/Muk1vnBum"); }

function Sc1() {
  window.open("https://ponselharian.com/8LECWnm6GVJx"); }

function Sc2() {
  window.open("https://ponselharian.com/uAw"); }

function Sc3() {
  window.open("https://ponselharian.com/fkL6rmJFiQO"); }

function Sc4() {
  window.open("https://ponselharian.com/UgV6k"); }

function Sc5() {
  window.open("https://ponselharian.com/LXmmLLA4glKC"); }

function ScBu() {
  window.open("https://ponselharian.com/M2fbH"); }

/* ### LULUBOX NK ### */
function LbNk() {
  window.open("https://ponselharian.com/tB6TmHRDg1F"); }
function SubLN() {
  window.open("https://cararegistrasi.com/YRHJjplThsYb","_self"); }

function SFr2() {
  window.open("https://ponselharian.com/uHhbPn3OO"); }

function SFr3() {
  window.open("https://ponselharian.com/Opsu"); }

function SFr4() {
  window.open("https://ponselharian.com/6uh"); }

function SFr5() {
  window.open("https://ponselharian.com/IF9gXkA"); }

function SFr6() {
  window.open("https://ponselharian.com/IQQU0ZgRUJXj"); }

function SFr7() {
  window.open("https://ponselharian.com/nPBStcqTj"); }

function SFrBu() {
  window.open("https://ponselharian.com/pASGD"); }

/* ### INJECT SKIN ### */
function Injt() {
  window.open("https://ponselharian.com/ErxSVVf0"); }
function SubIS() {
  window.open("https://cararegistrasi.com/mzLRtlBPd0","_self"); }

/* ### BOOSTER APK ### */
function RbApk() {
  window.open("https://ponselharian.com/TS8pPpuL"); }
function SubRA() {
  window.open("https://cararegistrasi.com/kpPbT1pR","_self"); }
/* ### BOOSTER SC ### */
function RbSc() {
  window.open("https://ponselharian.com/KJwzBiqJq3"); }
function SubRS() {
  window.open("https://cararegistrasi.com/N9YSnN","_self"); }

/* ### DAMAGE SC ### */
function DuSc() {
  window.open("https://ponselharian.com/HjV7S"); }
function SubDuS() {
  window.open("https://cararegistrasi.com/RoZfIpc1DUaL","_self"); }

/* ### HOOK SC ### */
function HokSc() {
  window.open("https://ponselharian.com/H9FBqVXycG"); }
function SubHS() {
  window.open("https://cararegistrasi.com/0lvWlljWA","_self"); }
function LinkHokApk() { 
  window.open("mh-kn.html"); }
